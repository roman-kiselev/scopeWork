...
export const unitSlice = createSlice({
    ...
    extraReducers(builder) {
        ...
        builder.addMatcher(
            unitsApi.endpoints.createUnit.matchFulfilled, (state, action) => {
        ...
        builder.addMatcher(
            unitsApi.endpoints.createUnit.matchRejected, (state, action) => {
                state.isLoading = false
                state.isError = true
        ...
        builder.addMatcher(
            unitsApi.endpoints.getAllUnits.matchFulfilled, (state, action) => {
                const data = action.payload
        ...
        builder.addMatcher(
            unitsApi.endpoints.getAllUnits.matchRejected, (state, action) => {
                state.isLoading = false
                state.isError = true
                const { data, status } = action.payload as IDataError
                state.dataError = {
